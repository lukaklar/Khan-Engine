...
namespace Khan
{
	...
	void TestPass::Setup(RenderGraph& renderGraph, Renderer& renderer)
	{
		...
		Texture* temp = renderer.GetResourceBoard().m_Transient.m_GBuffer.m_Depth; //renderGraph.CreateManagedResource(desc);

		viewDesc.m_Format = temp->GetDesc().m_Format;
		m_DepthBuffer = renderGraph.DeclareResourceDependency(temp, viewDesc, ResourceState_DepthWriteStencilWrite, true);
	}
