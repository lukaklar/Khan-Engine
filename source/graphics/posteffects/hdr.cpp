...
namespace Khan
{
	...
	void HDRPass::Setup(RenderGraph& renderGraph, Renderer& renderer)
	{
		...
		m_LightAccumulationBuffer = renderGraph.DeclareResourceDependency(renderer.GetResourceBlackboard().m_LightAccumulationBuffer, viewDesc, ResourceState_NonPixelShaderAccess);

		viewDesc.m_Format = renderer.GetResourceBlackboard().m_FinalOutput->GetDesc().m_Format;
		m_FinalOutput = renderGraph.DeclareResourceDependency(renderer.GetResourceBlackboard().m_FinalOutput, viewDesc, ResourceState_UnorderedAccess);
	}
