...
namespace Khan
{
	...
	BufferView* VulkanTransientResourceManager::FindOrCreateBufferView(const RenderPass* pass, Buffer* buffer, const BufferViewDesc& desc)
	{
		...
		KH_ASSERT(m_BufferViewPoolIndex < K_BUFFER_VIEW_POOL_SIZE, "Buffer view pool too small, you should increase its size.");
		VulkanBuffer* dummyBuffer = new(&m_DummyBufferPool[m_BufferViewPoolIndex])VulkanBuffer(*reinterpret_cast<VulkanBuffer*>(buffer));
		VulkanBufferView* value = new(&m_BufferViewPool[m_BufferViewPoolIndex++])VulkanBufferView(VK_NULL_HANDLE, *buffer, desc);
	...
	TextureView* VulkanTransientResourceManager::FindOrCreateTextureView(const RenderPass* pass, Texture* texture, const TextureViewDesc& desc)
	{
		...
		KH_ASSERT(m_TextureViewPoolIndex < K_TEXTURE_VIEW_POOL_SIZE, "Texture view pool too small, you should increase its size.");
		VulkanTexture* dummyTexture = new(&m_DummyTexturePool[m_TextureViewPoolIndex])VulkanTexture(*reinterpret_cast<VulkanTexture*>(texture));
		VulkanTextureView* value = new(&m_TextureViewPool[m_TextureViewPoolIndex++])VulkanTextureView(imageView, *texture, desc);
	...
	{
		for (uint32_t i = 0; i < m_BufferViewPoolIndex; ++i)
		{
			vkDestroyBufferView(m_Device.VulkanDevice(), m_BufferViewPool[i].GetVulkanBufferView(), nullptr);
			m_BufferViewPool[i].~VulkanBufferView();
		}
		m_BufferViewPoolIndex = 0;
		...
		for (uint32_t i = 0; i < m_TextureViewPoolIndex; ++i)
		{
			vkDestroyImageView(m_Device.VulkanDevice(), m_TextureViewPool[i].VulkanImageView(), nullptr);
			m_TextureViewPool[i].~VulkanTextureView();
		}
		m_TextureViewPoolIndex = 0;
