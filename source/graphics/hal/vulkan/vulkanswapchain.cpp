...
namespace Khan
{
	...
	void VulkanSwapchain::CreateSwapchain(uint32_t width, uint32_t height, bool vsync, bool hdr)
	{
		...
		swapchainInfo.imageUsage = VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_STORAGE_BIT; // TODO: Maybe add transfer src in order to take screenshots and definitely add sampled bit for posteffects
		...
		textureDesc.m_Flags = TextureFlag_AllowRenderTarget | TextureFlag_AllowUnorderedAccess; // TODO: Add flags here if it will be used for unordered access, shader resource etc.
